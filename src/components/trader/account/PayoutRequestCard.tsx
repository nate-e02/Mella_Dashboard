...
export function PayoutRequestCard({ fundedAccounts }: { fundedAccounts: FundedAccount[] }) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) throw new Error(body.error || body.issues?.[0]?.message || "Could not request payout");
      toast.push("Payout requested. You will be notified when it is approved.", "success");
      setAmount("");
      await loadPayouts();
    } catch (err) {
      toast.push(err instanceof Error ? err.message : "Could not request payout", "error");
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    <div className="card flex flex-col gap-4 p-5">
      <div>
        ...
        <p className="text-xs text-muted">Profit share from funded accounts is paid to telebirr, CBE Birr or a bank account within 48 hours of approval.</p>
      </div>

      {fundedAccounts.length === 0 ? (
        <p className="text-sm text-muted">Payouts become available once you have a funded account.</p>
      ) : (
        <form onSubmit={submit} className="flex flex-col gap-3 text-sm">
          ...
          {availability && (
            <div className="rounded-lg border border-border bg-surface-2 px-3 py-2 text-xs text-muted">
              <div>
                Profit {formatCurrency(availability.profit, "ETB")} · your share ({availability.profitSplitPercent}%) {formatCurrency(availability.traderShare, "ETB")} · already requested{" "}
                {formatCurrency(availability.alreadyCommitted, "ETB")}
              </div>
              <div className="mt-1 font-medium text-foreground">Available now: {formatCurrency(availability.available, "ETB")}</div>
              {!availability.kycApproved && <div className="mt-1 text-warning">Identity verification (KYC) must be approved first.</div>}
              {availability.fundedDays < availability.minFundedDays && (
                <div className="mt-1 text-warning">
                  First payout is available {availability.minFundedDays} days after funding ({availability.fundedDays} so far).
                </div>
              )}
              {availability.available < availability.minPayout && <div className="mt-1">Minimum payout is {formatCurrency(availability.minPayout, "ETB")}.</div>}
            </div>
          )}
          <label className="flex flex-col gap-1">
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
            ...
            <label className="flex flex-col gap-1">
              <span className="font-medium">{destType === "BANK" ? "Account number" : "Phone number"}</span>
